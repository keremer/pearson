...
class User(Base):
    ...
    def has_permission(self, action: str) -> bool:
        ...
        # 1. Define the matrix with explicit List typing
        # Moving this here or making it a class constant helps Pylance
        ...
        # 2. Get the role and ensure it is a plain string
        # Using 'getattr' is safe for SQLAlchemy models
        role_value = getattr(self, 'role', 'viewer')
        role_str = str(role_value) if role_value else 'viewer'

        # 3. The "Secret Sauce" for Pylance: 
        # Provide an explicitly typed empty list as the default.
        fallback: List[str] = [] 
        
        # This matches 'Overload 3' perfectly: get(str, List[str]) -> List[str]
        user_perms = permissions.get(role_str, fallback)
